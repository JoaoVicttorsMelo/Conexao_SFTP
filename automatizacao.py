...
while True:
    # Função para verificar a data de modificação do arquivo
    def verificar_data_modificacao(arquivo):
        data_modificacao = datetime.fromtimestamp(os.path.getmtime(arquivo))
        return data_modificacao.date() == datetime.today().date()
    ...
    def listar_e_transferir_arquivos():
        try:
            ...
            # Listando os arquivos .ret modificados hoje
            arquivos_para_transferir = []
            arquivos_enviados = []
            for arquivo in os.listdir(local_directory):
                if arquivo.endswith('.ret'):
                    caminho_arquivo = os.path.join(local_directory, arquivo)

                    if verificar_data_modificacao(caminho_arquivo):
            ...
            if arquivos_para_transferir:
                print("Arquivos modificados hoje encontrados:")
                ...
                for arquivo in arquivos_para_transferir:
                    try:
                        ...
                        if not verificar_arquivo_existe(sftp, arquivo, remote_directory_in) and \
                            ...
                            print(
                                f"Arquivo {arquivo} já existe em {remote_directory_in} ou {remote_directory_done}, não foi transferido.")
                            registrar_log(
                                f"Arquivo {arquivo} já existe em {remote_directory_in} ou {remote_directory_done}, não foi transferido.")
                    except Exception as e:
                ...
                print("Nenhum arquivo .ret modificado hoje foi encontrado.")
                registrar_log("Nenhum arquivo .ret modificado hoje foi encontrado.")
